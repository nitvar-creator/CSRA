...
const STEP = "delivered_baby_health_3" as const;
...
export default function MotherDeliveredBabyHealth3Page() {
  const router = useRouter();
  const hydrated = useHydrated();
  const [answers, setAnswers] = useState<MotherAnswers>(() => readDraftAnswers());
  const [feedingDifficulty, setFeedingDifficulty] = useState<string>(() =>
    boolToYesNo(readDraftAnswers().baby_feeding_difficulty),
  );
  ...
  const handleContinue = () => {
    setError(null);
    if (feedingDifficulty !== "yes" && feedingDifficulty !== "no") {
      setError("Please answer the question.");
      return;
    }
    const updated: MotherAnswers = {
      ...answers,
      baby_feeding_difficulty: feedingDifficulty === "yes",
    };
    setAnswers(updated);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-6 py-10 pb-20">
      ...
      <div className="mt-8 space-y-6">
        <QuestionCard title="Has the baby had difficulty feeding?" required>
          <BigRadioGroup
            name="baby_feeding_difficulty"
            value={feedingDifficulty}
            onChange={setFeedingDifficulty}
            required
            options={[
