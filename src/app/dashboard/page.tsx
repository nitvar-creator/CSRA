...
function countDistinct<T>(rows: { [k: string]: T }[] | null, key: string): number {
  if (!rows) return 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const r of screeningPositiveRes.data ?? []) {
        if (r.maternal_basic_id) affectedSet.add(r.maternal_basic_id);
      }
      ...
      setStats({
        ...
        affectedMothers: affectedSet.size,
